...
def scattered_section_contour_from_facets(p, t, facets, normals, magnification_ratio):
    ...
    SEARCH_RANGE_UPPER = SEARCH_RANGE_LOWER = max(
        abs(tan(radians(helpers_geom.degree_between_vectors(t, vector([0, 0, 1])))) * IMAGE_WIDTH), 0.5)
...
def test2():
    ...
    segments = ordered_outlines_from_contour_points(p, closing_distance_limit=1.5)
